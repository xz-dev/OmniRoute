...
test("runMigrations applies canonical 142 API-key model access on a fresh schema", serial, async () => {
  ...
  try {
    ...
    assert.equal(columns.some((column) => column.name === "model_access_mode"), true);
    assert.deepEqual(
...
test("runMigrations reconciles the production 134/135 ledger before filling canonical slots", serial, async () => {
  const runner = await importFresh("src/lib/db/migrationRunner.ts");
  const db = new Database(":memory:");
  try {
    db.exec(`
    ...
    const count = withMockedMigrationFs(
      {
        ...
        "142_api_keys_model_access_mode.sql": "ALTER TABLE api_keys ADD COLUMN model_access_mode TEXT;",
      },
      () => runner.runMigrations(db)
    );

    assert.equal(count, 1, "only the newly freed canonical 134 slot should execute");
    const rows = db.prepare("SELECT version, name FROM _omniroute_migrations ORDER BY version").all() as Array<{
      version: string;
      name: string;
    }>;
    assert.deepEqual(rows.map((row) => `${row.version}:${row.name}`), [
    ...
    const proxyColumns = db.prepare("PRAGMA table_info(proxy_logs)").all() as Array<{ name: string }>;
    assert.equal(proxyColumns.some((column) => column.name === "egress_ip"), true);
  } finally {
    db.close();
  }
});
