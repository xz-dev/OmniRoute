...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.equal(
        apiKeyColumns.filter((column) => column.name === "model_access_mode").length,
        1,
        "ledger reconciliation must not repeat the API-key ALTER"
      );
      const proxyLogColumns = db.prepare("PRAGMA table_info(proxy_logs)").all() as Array<{
        name: string;
      }>;
      assert.equal(proxyLogColumns.some((column) => column.name === "egress_ip"), true);
    } finally {
      db.close();
    }
  }
