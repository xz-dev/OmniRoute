/**
 * Acceptance: migration 143 — api_keys.model_access_mode
 *
 * Confirmed backfill:
...
const MIGRATION_143_PATH = path.join(
  __dirname,
  "../../src/lib/db/migrations/143_api_keys_model_access_mode.sql"
);
...
test("R-migration: 143_api_keys_model_access_mode.sql must exist", () => {
  assert.ok(
    fs.existsSync(MIGRATION_143_PATH),
    "expected src/lib/db/migrations/143_api_keys_model_access_mode.sql"
  );
});

test("R-migration: 143 adds model_access_mode and backfills all/restricted from allowed_models", () => {
  assert.ok(
    fs.existsSync(MIGRATION_143_PATH),
    "expected src/lib/db/migrations/143_api_keys_model_access_mode.sql"
  );

  const sql = fs.readFileSync(MIGRATION_143_PATH, "utf-8");
