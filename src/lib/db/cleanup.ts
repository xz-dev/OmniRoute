...
export async function cleanupCompressionRunTelemetry(): Promise<CleanupResult> {
  ...
  // This telemetry table is intentionally created lazily on the first recorded
  // compression run. A fresh install can reach startup cleanup before that, so
  // absence means there is nothing to retain—not a cleanup failure.
  if (!hasTable(db, "compression_run_telemetry")) {
    console.log("[Cleanup] Skipped compression_run_telemetry (table not initialized)");
    return result;
  }
